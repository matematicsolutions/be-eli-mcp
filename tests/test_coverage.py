...
from be_eli_mcp.coverage import build_coverage
from be_eli_mcp.server import be_coverage


def _coverage():
    fn = getattr(be_coverage, "fn", be_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BE_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("be_coverage" in line for line in lines), (
        "an audit entry was written but it does not name be_coverage"
    )
