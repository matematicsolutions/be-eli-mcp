...
from be_eli_mcp.server import INSTRUCTIONS, ToolError, mcp

SRC = (Path(__file__).parent.parent / "src" / "be_eli_mcp" / "server.py").read_text(
    encoding="utf-8"
)
...
def _referenced_tool_names_in_instructions() -> set[str]:
    skip = {"isError", "true", "false", "xml", "html"}
    out: set[str] = set()
...
def test_instructions_only_reference_registered_tools():
    ...
    referenced_tools = {r for r in referenced if r.startswith("be_")}
